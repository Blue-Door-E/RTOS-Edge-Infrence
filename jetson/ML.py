"""
ML.py — PyTorch model loader and inference engine.

Loads a TorchScript ResNet-18 model and runs single-image classification.
Supports both CUDA (Jetson GPU) and CPU fallback.  Gracefully degrades
...
def find_latest_model(model_dir: str = "outputs") -> Optional[Tuple[str, str]]:
    """Search for the most recently modified TorchScript model + labels file."""
    try:
        ...
        for d in subs:
            m = os.path.join(d, "resnet18_scripted.pt")
            l = os.path.join(d, "labels.txt")
            if os.path.exists(m) and os.path.exists(l):
                return m, l
        m = os.path.join(model_dir, "resnet18_scripted.pt")
...
class OptionalModel:
    """
    Wrapper around a TorchScript classification model.

    If PyTorch or the model files are missing, predict() returns
    a safe default instead of raising errors.
    """
    ...
    def predict(self, bgr: np.ndarray) -> Tuple[str, float, int]:
        """Run inference on a BGR image and return (label, confidence, index)."""
