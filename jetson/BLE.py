"""
BLE.py — Dual Bluetooth Low Energy client for smart-glasses communication.

Manages two simultaneous BLE connections (left + right arms of the glasses)
using the Nordic UART Service (NUS).  Handles MTU negotiation, chunked
writes, automatic reconnection, and service-discovery edge cases across
different Bleak / BlueZ versions.
"""
...
class DualBLE:
    """Manages BLE connections to both arms of the smart-glasses."""
    ...
    def __init__(self, left_id: str, right_id: str):
        ...
        self._ble_dead = asyncio.Event()
        self._reconnect_lock = asyncio.Lock()
        self._payload = 20

    # ------------------------------------------------------------------
    # MTU / payload negotiation
    # ------------------------------------------------------------------
    ...
    async def _wait_for_char(self, client: BleakClient, uuid: str,
                             tries: int = 40, delay: float = 0.15):
        for _ in range(tries):
            await self._force_discovery(client)
            if await self._has_char(client, uuid):
                return
            await asyncio.sleep(delay)
        print(f"[BLE] Warning: characteristic not visible yet: {uuid}")

    async def _wait_for_services(self, client: BleakClient, *,
                                 tries: int = 20, delay: float = 0.15):
        await self._force_discovery(client)
        for _ in range(tries):
            svcs = getattr(client, "services", None)
            if svcs is not None:
                ...
                if inner is None:
                    ...
                    try:
                        ...
                        pass
            await asyncio.sleep(delay)
        print("[BLE] Warning: services not populated (continuing).")

    async def _connect_one(self, addr: str, timeout: float = 30.0,
                           attempts: int = 8) -> BleakClient:
    ...
    async def _write_pkt(self, client: BleakClient, char_uuid: str,
                         pkt: bytes, *, timeout: float = 1.5):
        if not client or not getattr(client, "is_connected", False):
            raise RuntimeError("BLE client not connected")
        if len(pkt) > self._payload:
            raise RuntimeError(f"Packet too large: {len(pkt)} > {self._payload}")
    ...
    async def send_both(self, pkt: bytes):
        ...
        await self._wait_left_ack(timeout=0.35)
    ...
    async def text_0x4E(self, text: str):
        ...
        header_len = 9
        max_data = max(1, self._payload - header_len)
        ...
        for idx, part in enumerate(parts):
            ...
            pkt = hdr + part
            if len(pkt) > self._payload:
                raise RuntimeError(f"Packet {len(pkt)} > payload {self._payload}")
            await self.send_both(pkt)
    ...
    async def safe_text_0x4E(self, text: str):
        """Write text with automatic reconnection on failure."""
        try:
